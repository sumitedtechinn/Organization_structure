    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex align-items-center justify-content-between mb-3">
                ...
                <div class="d-flex justify-content-end gap-2 col-sm-6"></div>
            </div>
            <div class="row">
            ...
            <div class="table-responsive mt-3">
                <table class="table align-middle w-100" id="attendanceUserTable" style="color: #515B73!important;">
                    <thead class="table-primary">
                        <tr class="table_heading"> 
                            <td class="text-center" id="month">Month</td>
                        </tr>
                    </thead>
                </table>
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
    makeOrdinalColumn();
    getAllFilterData();
    column = Array.from(document.querySelectorAll("#attendanceUserTable thead.table-primary tr.table_heading td")).reduce((acc,param) => {
        let obj = {
            data : param.id , 
            render : (data,type,row) => (param.id == 'month') ? makeMonthColumnCell(data) : makeSingleCell(data)  
        };
        acc.push(obj);
        return acc;
    } , []);
    attendanceUserSetting.columns = column;
    $("#attendanceUserTable").DataTable(attendanceUserSetting);
})

var attendanceUserSetting = {
    'processing' : true,
    'serverSide' : true,
    'serverMethod' : 'POST',
    'searching' : false,
    ...
    pageLength: 6 ,
    'ajax': {
        url : '/app/attendance_system/attendance/attendanceUserServer', 
        type : 'POST',
        data : function(d) {
...
function makeMonthColumnCell(month_name) {
    return `<div class="d-flex flex-column align-items-center gap-1 cursor-pointer">
        <div class = "table_heading" style="color:#676363">${month_name}</div>
    </div>`;
}
