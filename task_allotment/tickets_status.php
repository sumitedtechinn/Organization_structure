        ...
        <div class="card-body">
            <div class="d-flex align-items-center justify-content-between">
                ...
                <div class="d-flex justify-content-end gap-2 col-sm-6">
                    <?php if(in_array('Tickets Status Delete',$_SESSION['permission'])) { ?>
                    ...
                    <button class="btn btn-primary" style="font-size: small;" id ="return_button">Go To Status</button>
                    <?php } ?>
                    <?php if(in_array('Tickets Status Create',$_SESSION['permission'])) { ?>
                    <div class="d-flex align-items-center theme-icons shadow-sm p-2 cursor-pointer rounded gap-2 bg-primary" title="Add Status" style="color: white;" onclick="addNewStatus()" data-bs-toggle = "tooltip"><span>Add Status</span>
                        <i class="bi bi-plus-circle-fill" id="add_new_status"></i>
                    </div>
                    <?php } ?>
                </div>
            </div>
            <div class="table-responsive mt-3">
                <table class="table align-middle w-100" id="ticketStatusTable">
...
var ticketStatusSetting = {
    'processing': true,
    'serverSide': true,
    'serverMethod': 'post',
    'ajax': {
        'url': '/app/ticketStatus/ticketStatus-server', 
        'type': 'POST',
    },
    'columns': [
        {
            ...
            data : "color",
            render : function(data,type,row) {
                return '<div style = "background-color : '+data+';height:15px;width : 15px;"> </div>';
            }
        },
        {
            data : "Action",
            render : function(data, type, row) {
                var edit = '';
                var del = '';
                <?php if(in_array('Tickets Status Update',$_SESSION['permission'])) { ?>
                ...
                <?php } ?>
                <?php if(in_array('Tickets Status Delete',$_SESSION['permission'])) { ?>
                    del = '<div class="text-danger" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete" onclick = "checkAssignDetails('+row.ID+',&#39;ticket_status&#39;)"><i class="bi bi-trash-fill"></i></div>';
                <?php } else { ?>
...
var ticketStatusTrashSettings = {
    ...
    'ajax': {
        ...
        'type': 'POST',
        'data' : function (d) {
            d.ticketStatusType = 'deleteTicketStatus'; 
        }
    },
    'columns': [{
            data: "sqNo",
...
$(document).ready(function() {
    ...
    $('#ticketStatusTable').dataTable(ticketStatusSetting);
});

$("#trash_button").on('click',function(){
    $('#ticketStatusTable').dataTable(ticketStatusTrashSettings);
...
async function addNewStatus() {
    const data = await getMethod("/app/ticketStatus/insertAndUpdateTicketStatus");
    if( data != null) {
...
async function updateDetails(ticketStatus_id) {
    const data = await postMethod("/app/ticketStatus/insertAndUpdateTicketStatus",{ticketStatus_id});
    if( data != null) {
        $('#md-modal-content').html(data);
        $('#mdmodal').modal('show');
    }
