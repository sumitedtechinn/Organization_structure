...
if($assets->num_rows > 0) {
    while ($row = mysqli_fetch_assoc($assets)) {
        if($row['user_id'] != 'Not Assign') {
            ...
            $user_id = $row['user_id'];
            $userDetails_query = "SELECT users.Name , Department.department_name FROM `users` LEFT JOIN Department ON Department.id = users.Department_id WHERE users.ID = '$user_id'";
            $userDetails = $conn->query($userDetails_query);
            $userDetails = mysqli_fetch_assoc($userDetails);
            $assets_assign_to = $userDetails['Name'];
        } else {
