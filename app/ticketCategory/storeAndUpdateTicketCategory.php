...
if(isset($_REQUEST['name']) && isset($_REQUEST['department']) && isset($_REQUEST['multiple_assignation'])) {
    ...
    $erpRole = null;
    if (isset($_REQUEST['erpRole'])) {
        $erpRole = json_encode($_REQUEST['erpRole']); 
    }

    if(!isset($_REQUEST['ID'])) {
        $insert_query = "INSERT INTO `ticket_category`(`name`, `department`,`multiple_assignation`,`erpRole`) VALUES ('$name','$department','$multiple_assignation','$erpRole')";
        $insert = $conn->query($insert_query);
        echo showResponse($insert,"Category added");
    } else {
        $id = mysqli_real_escape_string($conn,$_REQUEST['ID']);
        $update_query = "UPDATE `ticket_category` SET `name`= '$name',`department` = '$department' , `multiple_assignation` = $multiple_assignation  , `erpRole` = '$erpRole' WHERE id = '$id'";
