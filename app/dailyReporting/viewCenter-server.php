...
if (isset($_REQUEST['center_ids'])) {

    $center_ids = mysqli_real_escape_string($conn,$_REQUEST['center_ids']);
    ## Read value
    $draw = $_POST['draw'];
    $row = $_POST['start'];
    ...
    $all_count = $conn->query("SELECT COUNT(ID) as `allcount` FROM Closure_details WHERE Closure_details.id IN ($center_ids)");
    $records = mysqli_fetch_assoc($all_count);
    $totalRecords = $records['allcount'];

    ## Total number of record with filtering
    $filter_count = $conn->query("SELECT COUNT(ID) as `filtered` FROM Closure_details WHERE Closure_details.id IN ($center_ids) $searchQuery");
    $records = mysqli_fetch_assoc($filter_count);
    $totalRecordwithFilter = $records['filtered'];

    ## Fetch Record
    $closure_details = $conn->query("SELECT Closure_details.* , Projection_type.Name as `projection_type` FROM Closure_details LEFT JOIN Projection_type ON Projection_type.ID = Closure_details.projectionType WHERE Closure_details.id IN ($center_ids) $searchQuery $orderby LIMIT $row , $rowperpage");
    ...
    if ($closure_details->num_rows > 0 ) {
        while( $row = mysqli_fetch_assoc($closure_details)) {
            ...
            if(!is_null($row['Deleted_At'])) {
                $status = "Center Delete";
                $last_updated_date = $row['Deleted_At'];
            } else {
