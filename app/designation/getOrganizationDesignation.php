...
if(isset($_REQUEST['organization_id'])) {
    ...
    if (isset($_REQUEST['hierarchy_value']) && !empty($_REQUEST['hierarchy_value'])) {
        ...
        $parent = $conn->query("SELECT ID FROM `Designation` where hierarchy_value = '".$parent_hierarchy."' AND organization_id = '".$_POST['organization_id']."' AND branch_id IS NULL"); 
    ...
    $designation = $conn->query("SELECT ID , hierarchy_value , CONCAT(designation_name,'(',code,')') as `name` FROM Designation WHERE branch_id IS NULL AND organization_id = '".$_POST['organization_id']."'");
